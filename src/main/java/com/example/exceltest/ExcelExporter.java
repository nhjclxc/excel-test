...
public class ExcelExporter {
    protected static final Logger log = LoggerFactory.getLogger(ExcelExporter.class);
...
//        InputStream resourceAsStream = ExcelExporter.class.getClassLoader().getResourceAsStream("simple-template.xlsx");
        InputStream resourceAsStream = ExcelExporter.class.getClassLoader().getResourceAsStream("dynamic-template.xlsx");
        // 输出文件路径
        ...
        if (listValueStartRowIndex != -1) {
            ...
            int moveStart = listValueStartRowIndex + 1;//4  rowIndex=3
            int moveEnd = allRowList.size() - 1; // allRowList.size()=4
            int moveSize = dataList.size() + 1;
            // moveStart和moveEnd是前闭后闭，
            // 把原始list填充后面的数据先往最后移动
            sheet.shiftRows(moveStart, moveEnd, moveSize);
